...
class SteadyFP:
    ...
    def __init__(self, N, dx):
        ...
        if self.ndim == 1:
            ...
            # Fourier frequencies
            self.k = [2 * np.pi * fftfreq(self.N[i], dx[i]) for i in range(self.ndim)]
            self.idx = np.zeros(
            ...
            for m in range(self.N[0]):
                for n in range(self.N[1]):
                    self.idx[0, m, n, :, :] = (
                        m - np.tile(np.arange(self.N[0]), [self.N[1], 1]).T
                    )
                    self.idx[1, m, n, :, :] = n - np.tile(
    ...
    def precompute_operator(self, f, a):
        ...
        if self.ndim == 1:
            ...
            self.A = (
                -1j
                * np.einsum(
                    "i,ijkl->ijkl", self.k[0], f_hat[0, self.idx[0], self.idx[1]]
                )
                - 1j
                * np.einsum(
                    "j,ijkl->ijkl", self.k[1], f_hat[1, self.idx[0], self.idx[1]]
                )
                - np.einsum(
                    "i,ijkl->ijkl", self.k[0] ** 2, a_hat[0, self.idx[0], self.idx[1]]
                )
                - np.einsum(
                    "j,ijkl->ijkl", self.k[1] ** 2, a_hat[1, self.idx[0], self.idx[1]]
                )
            )
...
class AdjFP:
    ...
    def derivs1d(x):
        ...
        Dxx[-1, -4:] = [1.25, -2.75, 1.75, -0.25]
        Dxx[0, :4] = [-0.25, 1.75, -2.75, 1.25]
    ...
    @staticmethod
    def derivs2d(x, y):
        hx, hy = x[1] - x[0], y[1] - y[0]  # dphi, dsigma
        Nx, Ny = len(x), len(y)  # 50, 25
    ...
    def __init__(self, x, ndim=1):
        ...
        if self.ndim == 1:
            ...
            self.x = x  # [phi_range, sigma_range]
            self.N = [len(x[i]) for i in range(len(x))]  # [50, 25]
            self.dx = [x[i][1] - x[i][0] for i in range(len(x))]  # [dphi, dsigma]
    ...
    def operator2d(self, f, a):
        self.L = (
            sparse.diags(f[0]) @ self.Dx
            + sparse.diags(f[1]) @ self.Dy
            + sparse.diags(a[0]) @ self.Dxx
            + sparse.diags(a[1]) @ self.Dyy
        )
