...
def kl_divergence(p_in, q_in, dx: float | list[float] = 1, tol=None):
    """
...
def KM_avg(X, bins, stride, dt):
    ...
    for i in range(len(bins) - 1):
        ...
        if len(mask) > 0:
            ...
            f_err[i] = np.std(dY[mask]) / np.sqrt(len(mask))  # Conditional average
...
def SSR_loop(opt_fun, params):
    ...
    f_expr, a_expr = params["f_expr"].copy(), params["a_expr"].copy()
    lib_f, lib_a = params["lib_f"].copy(), params["lib_a"].copy()
    Xi0 = params["Xi0"].copy()

    m = len(f_expr) + len(a_expr)
    ...
    for k in range(1, m - 1):
        ...
        print(f"Active f: {f_active}")
        print(f"Active a: {a_active}", flush=True)
...
def cost(Xi, params):
    ...
    lib_f, lib_a = params["lib_f"], params["lib_a"]
    N = params["N"]
...
def markov_test(X, lag, N=32, L=2):
    ...
    # Three-time PDFs
    p123, _ = np.histogramdd(
        np.array([X1, X2, X3]).T, bins=np.array([bins, bins, bins]), density=True
    )
    p123_markov = np.einsum("ij,jk->ijk", p12, pcond_23)

    # Chi^2 value
    # return ntrapz( (p123 - p123_markov)**2, [dx, dx, dx] )/(np.var(p123.flatten()) + np.var(p123_markov.flatten()))
