...
def km_log_bins(
    ...
    kmc, edges = _km_log_bins(timeseries, bins, powers, kernel, bw, tol, conv_method)
...
def _km_log_bins(
    ...
    hist, edges = histogramdd(
        log_timeseries[:-1, ...], bins=bins, weights=weights, bw=bw
    )
    ...
    middles_exp = [np.exp((e[1:] + e[:-1]) / 2) for e in edges]
    # convert from P[log x] to P[x]
    kmc[0] /= middles_exp[0]

    exp_edges = [np.exp(e) for e in edges]
