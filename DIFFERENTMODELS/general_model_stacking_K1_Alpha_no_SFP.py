...
from utils_stack import cost_stack, cost_alpha_stack, cost_enforce_eqn_stack
...
def run_sindy_model(
    ...
    opt_func = partial(optimise_functions, cost_enforce_eqn_stack, cost_alpha_stack)
    ...
    if EXPLICIT:
        for i in range(len(Xis)):
            ...
            ax_pdf_comp.set_yscale("log")
            fig_pdf_comp.tight_layout()
            fig_pdf_comp.savefig(
                everymodel
                / f"{MODEL_NAME}_pdf_comparison_sparsity_{sparsity[i]}_log.png"
            )
        ...
        ax_pdf_comp.set_yscale("log")
        fig_pdf_comp.tight_layout()
        fig_pdf_comp.savefig(folderpath / f"{MODEL_NAME}_pdf_comparison_log.png")
        plt.close(fig_pdf_comp)
...
if __name__ == "__main__":
    ...
    int_param_group.add_argument(
        "--coeffs", type=float, nargs="+", default=[0.0, -0.016, 0.0, 1.0, -1.0]
    )
    args = df.parse_args()
    print(args)
