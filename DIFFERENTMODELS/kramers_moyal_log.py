...
from kramersmoyal.kernels import silvermans_rule, epanechnikov


def km_log_bins(
    timeseries: np.ndarray,
    bins: str | int | list | tuple = "default",
    powers: int | list | tuple | np.ndarray = 4,
    ...
    assert np.all(timeseries != 0), "In order to perform log all data must be != 0"
    ...
    if isinstance(bins, str):
        if bins == "default":
            assert 0 < dims < 4, "If dimension of timeseries > 3, set bins manually"
            if dims == 1:
                bins = [5000]
            elif dims == 2:
                bins = [100] * 2
            ...
            raise ValueError(
                f'Unrecognised string argument for bins, got {bins}, expected "Default" only.'
            )
    ...
    kmc, log_edges = _km_log_bins(
        timeseries, bins, powers, kernel, bw, tol, conv_method
    )

    if center_edges:
        log_edges = [
            log_edge[:-1] + 0.5 * (log_edge[1] - log_edge[0]) for log_edge in log_edges
        ]

    exp_edges = [np.exp(e) for e in log_edges]
    if not full:
        return (kmc, exp_edges)
    else:
        return (kmc, exp_edges, bw, powers)
...
def _km_log_bins(
    ...
    log_timeseries = np.log(np.abs(timeseries))

    # Get weighted histogram
    hist, log_edges = histogramdd(
        log_timeseries[:-1, ...], bins=log_bins, weights=weights, bw=bw
    )

    # Generate centred kernel on larger grid (fft'ed convolutions are circular)
    log_edges_k = [(e[1] - e[0]) * np.arange(-e.size, e.size + 1) for e in log_edges]
    kernel_ = kernel(cartesian_product(log_edges_k), bw=bw)
    ...
    # convert from log x to x
    middles_exp = [np.exp((e[1:] + e[:-1]) / 2) for e in log_edges]
    # convert from P[log x] to P[x]
    kmc[0] /= middles_exp[0]

    return kmc, log_edges
...
def _km_log_bins_2(
    timeseries: np.ndarray,  # shape == (N, D)
    bins: list[np.ndarray],  # list(shapes) == [(B1,), ..., (BD,)]
    which_log: list[bool],
    powers: np.ndarray,
    kernel: Callable,
    bw: float,
    ...
    for i, to_log in enumerate(which_log):
        if to_log:
            middles_exp = np.exp((edges[i][1:] + edges[i][:-1]) / 2)
            # convert from P[log x] to P[x]
            kmc[i][0] /= middles_exp[0]

    edges = [np.exp(e) if which_log[i] else e for i, e in enumerate(edges)]
